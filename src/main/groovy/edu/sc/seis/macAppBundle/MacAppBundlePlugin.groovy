...
class MacAppBundlePlugin implements Plugin<Project> {
    ...
    static final String TASK_BUNDLE_JDK_NAME = "bundleJDK"
    ...
    void apply(Project project) {
        ...
        project.afterEvaluate {
            ...
            Task bundleJDKTask = createBundleJDKTask(project)
            ...
            createAppTask.dependsOn(bundleJDKTask)
    ...
    private Task addCreateInfoPlistTask(Project project) {
        ...
        task.inputs.property("MacAppBundlePlugin bundleJDK", { project.macAppBundle.bundleJDK })
        ...
        if (project.macAppBundle.jdkHome) task.inputs.property("MacAppBundlePlugin jdkHome", { project.macAppBundle.jdkHome })
    ...
    private Task createBundleJDKTask(Project project) {
        Task task = project.tasks.create(TASK_BUNDLE_JDK_NAME, Sync)
        task.description = "Copies the JDK into the Contents/PlugIns directory."
        task.group = GROUP

        project.afterEvaluate {
            if (project.macAppBundle.bundleJDK && project.macAppBundle.getJdkHome() != null) {
                task.from("${-> project.macAppBundle.getJdkHome()}/") {
                    exclude('src.zip')
                    exclude('sample/**')
                    exclude('man/**')
                }
                task.into "${-> project.buildDir}/${-> project.macAppBundle.appOutputDir}/${-> project.macAppBundle.appName}.app/Contents/JDK/${-> project.macAppBundle.getJDKDirName()}"
            }
            task.onlyIf { project.macAppBundle.bundleJDK }
        }
        return task
    }
